...
class MononokeBTPlugin:
    ...
    def run_search(self, pattern, param, href=None, page=0):
        ...
        self.api.log("start")
        resp, content = httplib2.Http().request(href, headers={'Cookie': self.api.parse_cookie(self.api.get_login_cookie)})
        ...
        for i in lines:
            try:
                ...
                resp, content = httplib2.Http().request(link, headers={'Cookie': self.api.parse_cookie(self.api.get_login_cookie)})
                content = content.decode("utf-8")
