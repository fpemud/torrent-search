...
class ThePirateBayTorrentPluginResult(TorrentSearch.Plugin.PluginResult):
    ...
    def __init__(self, label, date, size, seeders, leechers, torrent_url, magnet_url, category, nb_comments, details_page_url):
        TorrentSearch.Plugin.PluginResult.__init__(
            self, label, date, size, seeders, leechers, magnet_url, category=category, nb_comments=nb_comments, orig_url=details_page_url)
    ...
    def _do_load_filelist(self):
        ...
        os.unlink(filename)
        files = TorrentSearch.htmltools.find_elements(
    ...
    def _do_load_comments(self):
        ...
        filename, msg = urllib.request.urlretrieve(self.details_page_url)
        tree = libxml2.htmlParseFile(filename, "utf-8")
        os.unlink(filename)
        div = TorrentSearch.htmltools.find_elements(
        ...
        comments_zone = TorrentSearch.htmltools.find_elements(
            tree.getRootElement(), "div", id="comments")
        if comments_zone:
            comments_zone = comments_zone[0]
            comments_browser = TorrentSearch.htmltools.find_elements(
                comments_zone, "div", **{'class': 'browse-coms noborder'})
            pages = []
            if comments_browser:
                comments_browser = comments_browser[0]
                links = TorrentSearch.htmltools.find_elements(
                ...
                if links:
                    ...
                    for i in links:
                        if not TorrentSearch.htmltools.find_elements(i, "img"):
                            ...
                            tree = libxml2.htmlParseFile(filename, "utf-8")
                            os.unlink(filename)
                            pages.append(TorrentSearch.htmltools.find_elements(
            ...
            while pages:
                page = pages[-1]
                del pages[-1]
                page_comments = TorrentSearch.htmltools.find_elements(
...
class ThePirateBayTorrentPlugin(TorrentSearch.Plugin.Plugin):
    ...
    def run_search(self, pattern, page_url=""):
        ...
        results_table = TorrentSearch.htmltools.find_elements(
            tree.getRootElement(), "table", id="searchResult")[0]
        ...
        maindiv = TorrentSearch.htmltools.find_elements(
            tree.getRootElement(), "div", id="main-content")[0]
        pager = maindiv.nextElementSibling()

        next_page_img = TorrentSearch.htmltools.find_elements(
            pager, "img", alt="Next")
        if next_page_img:
    ...
    def _parse_result(self, base_url, result_line):
        cat_cell, main_cell, seeders_cell, leechers_cell = TorrentSearch.htmltools.find_elements(
            result_line, "td")

        main_cat, sub_cat = TorrentSearch.htmltools.find_elements(
            cat_cell, "a")
        ...
        date_size_data = TorrentSearch.htmltools.find_elements(main_cell, "font")[
            0].getContent()
