...
class SUMOTorrentPluginResult:
    ...
    def _get_magnet(self, url):
        ...
        form = htmltools.find_elements(
            tree.getRootElement(), "form", id="frmAdultDisclaimer")
        if form:
            form = form[0]
    ...
    def _do_get_link(self):
        ...
        while self.reflink[i] != '/':
            i -= 1
        url = self.reflink[:i+1]+urllib.parse.quote_plus(self.reflink[i+1:])
    ...
    def _do_load_filelist(self):
        ...
        while self.details_url[i] != '/':
            i -= 1
        url = self.details_url[:i+1] + \
            urllib.parse.quote_plus(self.details_url[i+1:])
    ...
    def _do_load_comments(self):
        ...
        try:
            ...
            comments_table = htmltools.find_elements(htmltools.find_elements(
            ...
            for i in range(len(lines)/3):
                try:
                    ...
                    username_node = htmltools.find_elements(
                        title_line, "strong")[0]
                    ...
                    res.append(TorrentSearch.Plugin.TorrentResultComment(
...
class SUMOTorrentPlugin:
    ...
    def run_search(self, pattern, page=1, href=None):
        ...
        tree = libxml2.htmlParseDoc(content, "utf-8")
        try:
            count_div = htmltools.find_elements(htmltools.find_elements(
            ...
            try:
                if i.hasProp('class') and not i.hasProp('id'):
                    ...
                    refmagnet = urllib.basejoin(
                        href, htmltools.find_elements(name, "a")[0].prop('href'))
                    ...
                    nb_comments_zone = htmltools.find_elements(
                        comments, "strong")
                    nb_comments = 0
                    try:
